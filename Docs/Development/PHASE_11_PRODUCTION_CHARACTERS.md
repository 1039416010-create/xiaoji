...
> **方向变更（2026-09-20）**：本文件记录三维生产阶段及其历史验收。用户已决定停止
> 正式三维角色生产，改用已确认的角色细化图制作二维混合动画。当前执行文档为
> `PHASE_11_2D_CHARACTER_PRODUCTION.md`，决策依据见
> `Decisions/ADR-0003-2D-CHARACTER-PRESENTATION.md`。既有三维资产仅归档，不作为
> Release 正式角色。
...
- 13 套三视图、13 套动作表已经保存在 `Docs/Art/Concepts/Phase11/`。
- 闪电鸡和胖墩另有表情、材质及缺失动作补充页。
- `CHARACTER_ART_BIBLE.md` 已明确颜色、比例、配件、变形重点、动作签名和图像歧义处理规则。
- 用户已授权建模前连续完成角色美术，无需逐张等待审核。
- Phase 11A 已进入建模绑定：闪电鸡、胖墩、摸鱼鸡完成首轮工程灰模、46 骨共享层级、刚性分区蒙皮、Unity Generic Rig 验证 FBX 与 8 个 Blocking 动作。
- 第二版已补齐三档 LOD、统一 UV0、8 个表情 BlendShape 和 11 个 Blocking 动作槽，并通过冲刺、摔倒、庆祝极限姿势检查；配件 Socket 跟随与累积形变爆形问题已修复。
- 当前资产用于比例、轮廓、骨架兼容、LOD 和状态切换验证，不等同于 25k–45k 三角面的最终手工拓扑模型。
- 下一步在三个标杆第二版上完成细雕、手工重拓扑、PBR 材质、逐姿势修权重及 11 个完整性格动画；11A 退出条件尚未达成。

### 第三版制作检查点（2026-09-18）

- `Production/VisualV3/` 新增连续躯干/颈头网格、可见瞳孔与独立眼睑、冠羽/翼尖/尾羽，以及按原画调整的配饰。保留 v1/v2 源文件。
- 三只角色的 LOD0 分别为 29,732 / 34,388 / 34,556 三角面；保持 46 骨，增加为 9 个语义明确的表情形变。高度归一到 1.08 / 1.00 / 1.00 m。
- 自动打包 UV、2048² BaseColor/Normal/Mask 已产出；法线为程序凹凸烘焙，Mask AO 通道尚为常量 1，不应视为最终手绘材质或高模烘焙。
- 通过 99 个抽样姿势、表情独立性、蒙皮权重、Root 静止及 FBX 重新导入检查。对 Blocking 的地面穿透做了逐帧高度补偿，仍需处理脚滑、自穿插与性格动作。
- Unity 6000.3.24f1 实际导入成功：三份有效 Generic Avatar、各 9 个 BlendShape、11 个动作、三档 LOD，检查用 Prefab 的 `applyRootMotion=false`；没有替换正式比赛场景。
- 详见 `Verification/PHASE_11A_VISUAL_V3.md`。三视图的精细美术、手工重拓扑、控制 Rig、完整性格动画和投影实拍仍待完成，不能因结构验证通过宣告 Phase 11A 完成。
